...
test_open_append_fetch(void)
{
    ...
    CU_ASSERT(sb.st_size == 56);

    char correct_indexbin[] = {
        0x1d, 0x81, 0x09, 0x00, 0x9f, 0x9c, 0xf4, 0x3c, 0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
    ...
    char buf[56];
    CU_ASSERT(read(fd, buf, 56) == 56);
    CU_ASSERT(memcmp(buf, correct_indexbin, 56) == 0);
    ...
    char key[16] = {0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01,};
    CU_ASSERT(memcmp(ili->index_key->key, key, 16) == 0);
...
test_append_twice(void)
{
    ...
    char key2[16] = {0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,};
    CU_ASSERT(memcmp(ili->index_key->key, key2, 16) == 0);
